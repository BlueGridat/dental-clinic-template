      ...
      <MotionReveal>
        <div className="grid overflow-hidden rounded-[2rem] bg-[var(--color-accent)] p-4 md:p-0 lg:grid-cols-[1fr_420px]">
          <div className="p-3 text-center md:p-10 md:text-left lg:p-14">
            <h2 className="font-heading mx-auto max-w-3xl text-3xl font-bold leading-tight md:mx-0 md:text-5xl">{tr(banner?.heading) || tr({ de: "Erst beraten. Dann entscheiden.", en: "Talk first. Decide after." })}</h2>
            <p className="mx-auto mt-4 max-w-xl text-base leading-7 text-[var(--color-primary)]/75 md:mx-0 md:mt-5 md:text-lg md:leading-8">{tr(banner?.description)}</p>
            <ArrowButton href={banner?.cta?.href || "#book"} label={banner?.cta?.label || { de: "Erstgespräch anfragen", en: "Request a first consultation" }} variant="white" className="mt-7 w-full justify-center md:mt-8 md:w-auto" />
          </div>

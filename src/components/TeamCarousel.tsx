...
export function TeamCarousel({ team }: { team: TeamConfig }) {
  ...
  const doctors = useMemo(() => {
    ...
    return activeFilterIndex === 0 ? items : items.filter((doctor) => tr(doctor.category) === tr(filters[activeFilterIndex]));
  }, [activeFilterIndex, filters, team?.doctors, tr]);
      ...
      <div className="container-page rounded-[2rem] bg-[var(--color-white)] p-5 shadow-sm sm:p-8 lg:p-10">
          ...
          <div className="-mx-1 flex gap-2 overflow-x-auto px-1 pb-1 md:flex-wrap md:overflow-visible" role="tablist" aria-label="Doctor specialties">
            {filters.map((filter, index) => (
              <button
                ...
                type="button"
                role="tab"
                aria-selected={activeFilterIndex === index}
                className={cx("focus-ring min-h-11 shrink-0 rounded-full px-5 py-2 text-sm font-bold transition active:scale-[0.98] md:min-h-0 md:px-4 md:text-xs", activeFilterIndex === index ? "bg-[var(--color-accent)] text-[var(--color-primary)] md:bg-[var(--color-primary)] md:text-[var(--color-white)]" : "bg-[var(--color-surface)] text-[var(--color-primary)]")}
                onClick={() => {
        ...
        <div className="mt-7 flex items-center justify-between">
          ...
          <div className="flex gap-3">
            <button type="button" onClick={scrollPrev} className="focus-ring grid size-12 place-items-center rounded-full bg-[var(--color-white)]" aria-label={tr({ de: "Vorherige Ärzt:innen", en: "Previous doctors" })}>
              <ArrowLeft className="size-5" />
            </button>
            <button type="button" onClick={scrollNext} className="focus-ring grid size-12 place-items-center rounded-full bg-[var(--color-primary)] text-[var(--color-white)]" aria-label={tr({ de: "Nächste Ärzt:innen", en: "Next doctors" })}>

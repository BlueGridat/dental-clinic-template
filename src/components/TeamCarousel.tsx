    ...
    <section id="doctors" className="section-pad">
      <div className="container-page rounded-[2rem] bg-[var(--color-white)] p-5 shadow-sm sm:p-8 lg:p-10">
        <div className="mb-8 grid gap-6 lg:grid-cols-[0.9fr_0.65fr_auto] lg:items-end">
          ...
          <div className="flex flex-wrap gap-2" role="tablist" aria-label="Doctor specialties">
            {filters.map((filter) => (
              <button
                key={filter}
                type="button"
                ...
                className={cx("focus-ring rounded-full px-4 py-2 text-xs font-bold transition", activeFilter === filter ? "bg-[var(--color-primary)] text-[var(--color-white)]" : "bg-[var(--color-surface)] text-[var(--color-primary)]")}
                onClick={() => {
        ...
        <div className="overflow-hidden" ref={emblaRef}>
          <div className="flex gap-5">
            {doctors.map((doctor) => (
              <article key={`${doctor.name}-${doctor.role}`} className="min-w-0 flex-[0_0_78%] sm:flex-[0_0_38%] lg:flex-[0_0_24%]">

      ...
      {open ? (
        ...
        >
          <div className="flex items-center justify-between">
            <p className="font-heading text-xl font-bold">{nav?.cta?.label || "Menu"}</p>
            <button ref={closeRef} type="button" onClick={onClose} className="focus-ring grid size-12 place-items-center rounded-full bg-white/10" aria-label="Close menu">
              <X className="size-6" />
            </button>
          </div>
          ...
          <motion.div className="mt-12 grid gap-3" initial="hidden" animate="show" variants={{ hidden: {}, show: { transition: { staggerChildren: 0.06 } } }}>
            {safeArray(nav?.links).map((link) => (
              <motion.div key={`${link.label}-overlay`} variants={{ hidden: { opacity: 0, y: 16 }, show: { opacity: 1, y: 0 } }}>
          ...
          <div className="absolute inset-x-5 bottom-[calc(24px+env(safe-area-inset-bottom))] grid gap-3">
            ...
            <div className="grid grid-cols-2 gap-3 text-center text-sm font-bold">
              <a href={`tel:${contact?.phone || ""}`} className="focus-ring rounded-full bg-white/10 px-4 py-4 active:scale-[0.98]">Call</a>
              <a href={`mailto:${contact?.email || ""}`} className="focus-ring rounded-full bg-white/10 px-4 py-4 active:scale-[0.98]">Email</a>
            </div>
          </div>
        </motion.div>
      ) : null}

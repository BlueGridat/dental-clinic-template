      ...
      <div className="px-4">
        ...
        <div className="mt-6 grid gap-3">
          {safeArray(data?.items).map((item, index) => {
            ...
            return (
              <div key={`${item.title}-mobile`} className={cx("rounded-[1.5rem] p-2", item.highlight ? "bg-[var(--color-accent)]" : "bg-[var(--color-white)]")}>
                ...
                {open ? <p className="px-4 pb-4 text-sm leading-7">{item.text}</p> : null}
              </div>
            );
          })}
        </div>
